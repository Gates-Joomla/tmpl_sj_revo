...
function modChrome_ytmod($module, &$params, &$attribs){ ?>
	...
	if(strpos($params->get('moduleclass_sfx'), 'fa-')===false){
	  ...
	  if(strpos($modclass_sfx, "#" )){
		$color = strstr($modclass_sfx, "#");
		$border= "style='border-top: 2px solid $color'";
	  } else{
		 $border="";
		 //die();
	  }
	...
	<div <?php echo $border;?> class="module <?php echo $modclass_sfx ?> " <?php echo ($scrollreveal !='')?  'data-anijs="'. $scrollreveal.'"' : '' ; ?>>
	    <?php if ((bool) $module->showtitle) : ?>
			<?php if(strpos($modclass_sfx, "#" )){?>
		    <h3 class="modtitle" style="background: <?php echo $color;?>">
			<span>
			<?php echo $icons; ?><?php echo $module->title; ?><?php echo $badge; ?>
			</span>
			</h3>
			<?php } else{ ?>
			<h3 class="modtitle">
