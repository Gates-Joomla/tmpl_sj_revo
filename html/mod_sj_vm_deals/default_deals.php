					...
					if ($params->get('item_title_display', 1) == 1) {
					 ...
					 if ((int)$params->get('item_prices_display', 1) && ( !empty($list[0]->prices['salesPrice']) || !empty($list[0]->prices['salesPriceWithDiscount'])) ) { 
						echo '<div class="item-prices">';
							...
							echo '</div>';
							echo '<span class="PricebasePriceVariant">';
								echo $item->prices['product_price'];
							echo '</span>';
	...
	echo '<div class="owl-carousel owl-theme">';
		foreach($list as $index => $item){
			...
			echo '<div class="item">';
				...
				echo '<div class="item-inner product-container">';
					...
					if ($item_img) {
						echo '<div class="item-image">
								...
								if($params->get('item_popup_display') == 1){
									echo '<div class="sj_vm_deals_pop group-addtocart">';
										if ($params->get('item_addtocart_display', 1) == 1) {
											$_item['product'] = $item;
												echo shopFunctionsF::renderVmSubLayout('addtocart', $_item);
										}
										if ($params->get('item_quickview_display', 1) == 1) {
											echo '<div class="item-quick-view">';
												echo '<a href="'.$item->link.'" title="'.$item->title.'" '.VMDealsHelper::parseTarget($params->get('link_target')).'></a>';
											echo '</div>';
										}
									echo '</div>';
								}	
					...
					echo '<div class="main-right">';
						if ($params->get('item_title_display', 1) == 1) {
							echo '<h3 class="item-title">';
								echo '<a href="'.$item->link.'" title="'.$item->title.'" '.VMDealsHelper::parseTarget($params->get('link_target')).' >';
									$title_item= VMDealsHelper::truncate($item->title, (int)$params->get('item_title_max_characters', 25));
						...
						$des= substr($item->product_s_desc,0,150);
						echo '<p class="des">';
							echo $des;
						echo '</p>';
						...
						if ((int)$params->get('item_prices_display', 1) && ( !empty($item->prices['salesPrice']) || !empty($item->prices['salesPriceWithDiscount'])) ) {
							echo '<div class="item-prices">';
								...
								echo '<div class="price-old">';
									echo $currency->createPriceDiv('discountAmount', JText::_("Price: "), $item->prices['product_price'], false, false, 1.0, true);
								echo '</div>';
