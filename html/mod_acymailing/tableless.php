...
?><div class="acymailing" id="acymailing_module_<?php echo $formName; ?>">
<?php
	$style = array();
	if($params->get('effect','normal') == 'mootools-slide'){
		if(!empty($mootoolsIntro)) echo '<div class="acymailing_mootoolsintro">'.$mootoolsIntro.'</div>'; ?>
		<div class="acymailing_mootoolsbutton" id="acymailing_toggle_<?php echo $formName; ?>">
			<div><a class="acymailing_togglemodule" id="acymailing_togglemodule_<?php echo $formName; ?>" href="#subscribe"><?php echo $mootoolsButton ?></a></div>
	<?php
	}
		...
		<div class="acymailing_module_form" >
			...
			<div class="acymailing_form">
					...
					foreach($fieldsToDisplay as $oneField){
						if(empty($extraFields[$oneField])) echo '<div class="onefield fieldacy'.$oneField.'" id="field_'.$oneField.'_'.$formName.'">';
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							<span class="acyfield_<?php echo $oneField. (!$displayOutside? ' acy_requiredField':''); ?>"><input id="user_email_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'readonly="readonly" '; if(!$displayOutside){ ?>  placeholder="<?php echo $emailCaption; ?>"<?php } ?> class="inputbox" type="text" name="user[email]" style="width:<?php echo $fieldsize; ?>" value="" title="<?php echo $emailCaption;?>" /></span>
							<?php
							...
							if($extraFields[$oneField]->type == 'category'){
								...
								echo '<div class="onefield fieldacy'.$oneField.'" id="field_'.$oneField.'_'.$formName.'">';
								if($displayOutside){
						...
						if(empty($extraFields[$oneField])) echo '</div>';
					}
